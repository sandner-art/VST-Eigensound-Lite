...
const QuantumOrbitalApp = () => {
  ...
  // Enhanced audio synthesis parameters
  ...
  const [frequencyRange, setFrequencyRange] = useState({ min: 55, max: 880 });
  ...
  const colorPalettes = {
    scientific: {
      name: 'Scientific',
      colors: [
        { r: 0.0, g: 0.4, b: 0.8 }, { r: 0.0, g: 0.8, b: 0.8 }, { r: 0.0, g: 0.8, b: 0.4 },
        { r: 0.8, g: 0.8, b: 0.0 }, { r: 0.8, g: 0.4, b: 0.0 }, { r: 0.8, g: 0.0, b: 0.4 }
      ]
    },
    artistic: {
      name: 'Artistic',
      colors: [
        { r: 0.42, g: 0.27, b: 0.76 }, { r: 0.23, g: 0.51, b: 0.96 }, { r: 0.02, g: 0.71, b: 0.83 },
        { r: 0.06, g: 0.72, b: 0.51 }, { r: 0.96, g: 0.62, b: 0.04 }, { r: 0.94, g: 0.27, b: 0.27 }
      ]
    },
    neon: {
      name: 'Neon',
      colors: [
        { r: 1.0, g: 0.0, b: 0.5 }, { r: 0.5, g: 0.0, b: 1.0 }, { r: 0.0, g: 0.5, b: 1.0 },
        { r: 0.0, g: 1.0, b: 0.5 }, { r: 1.0, g: 0.5, b: 0.0 }, { r: 1.0, g: 0.0, b: 0.25 }
      ]
    },
    plasma: {
      name: 'Plasma',
      colors: [
        { r: 0.05, g: 0.11, b: 0.16 }, { r: 0.25, g: 0.35, b: 0.47 }, { r: 0.47, g: 0.55, b: 0.66 },
        { r: 0.88, g: 0.88, b: 0.87 }, { r: 1.0, g: 0.84, b: 0.04 }, { r: 1.0, g: 0.42, b: 0.21 }
      ]
    },
    rainbow: {
      name: 'Rainbow',
      colors: [
        { r: 0.58, g: 0.0, b: 0.83 }, { r: 0.29, g: 0.0, b: 0.51 }, { r: 0.0, g: 0.0, b: 1.0 },
        { r: 0.0, g: 1.0, b: 0.0 }, { r: 1.0, g: 1.0, b: 0.0 }, { r: 1.0, g: 0.5, b: 0.0 }, { r: 1.0, g: 0.0, b: 0.0 }
      ]
    }
  };
  ...
  const associatedLaguerre = (alpha, n, x) => {
    ...
    if (n === 4) return ((24 + 50*alpha + 35*alpha*alpha + 10*alpha*alpha*alpha + alpha*alpha*alpha*alpha - 24*(1 + 2*alpha + alpha*alpha + alpha*alpha*alpha/3)*x + 12*(2 + 3*alpha + alpha*alpha)*x*x - 4*(3 + alpha)*x*x*x + x*x*x*x) / 24);
  ...
  const probabilityDensity = (n, l, m, x, y, z, Z = 1) => {
    ...
    return Math.abs(psi * psi); // |ψ|² - probability density
  };

  // Musical scale functions
  const getScaleFrequency = (baseFreq, scaleStep) => {
    const scale = musicalScales[musicalScale];
    const rootFreq = noteFrequencies[rootNote] || 261.63; // Default to C
    
    const scaleLength = scale.intervals.length;
    const octave = Math.floor(scaleStep / scaleLength);
    const noteIndex = scaleStep % scaleLength;
    const semitoneOffset = scale.intervals[noteIndex];
    
    // Calculate frequency with proper octave scaling
    const freq = rootFreq * Math.pow(2, octave) * Math.pow(2, semitoneOffset / 12);
    
    // Constrain to frequency range
    return Math.max(frequencyRange.min, Math.min(frequencyRange.max, freq));
  };

  const quantumToScaleStep = (n, l, m, Z) => {
    // Map quantum numbers to scale steps
    const baseStep = (n - 1) * 7 + l * 2 + Math.abs(m);
    const zOffset = Math.floor((Z - 1) * 1.5);
    return baseStep + zOffset;
  };
  ...
  // Audio synthesis system with musical scales
  ...
  // Multiple synthesis modes with musical scales
  const createSynthesis = useCallback((n, l, m, Z) => {
    const audioContext = initAudioContext();
    const scaleStep = quantumToScaleStep(n, l, m, Z);
    const baseFreq = getScaleFrequency(220, scaleStep);

    switch (synthesisMode) {
      case 'oscillator':
        return createOscillatorSynth(audioContext, baseFreq, n, l, m, Z);
      case 'granular':
        return createGranularSynth(audioContext, baseFreq, n, l, m, Z);
      case 'modal':
        return createModalSynth(audioContext, baseFreq, n, l, m, Z);
      case 'cinematic':
      default:
        return createCinematicSynth(audioContext, baseFreq, n, l, m, Z);
    }
  }, [synthesisMode, musicalScale, rootNote, frequencyRange, volume, harmonicContent, spatialWidth, modulationDepth, initAudioContext]);

  const createOscillatorSynth = (audioContext, baseFreq, n, l, m, Z) => {
    const oscillators = [];
    
    for (let harmonic = 0; harmonic < Math.min(n, 5); harmonic++) {
      const osc = audioContext.createOscillator();
      const gain = audioContext.createGain();
      const filter = audioContext.createBiquadFilter();
      const panner = audioContext.createStereoPanner();
      
      const scaleStep = quantumToScaleStep(n, l, m, Z) + harmonic;
      const freq = getScaleFrequency(baseFreq, scaleStep);
      ...
      gain.gain.setValueAtTime(volume * 0.3 / (harmonic + 1), audioContext.currentTime);
  ...
  const createGranularSynth = (audioContext, baseFreq, n, l, m, Z) => {
    const grainScheduler = () => {
      ...
      const randomScaleStep = quantumToScaleStep(n, l, m, Z) + Math.floor(Math.random() * 7);
      const freq = getScaleFrequency(baseFreq, randomScaleStep);
  ...
  const createModalSynth = (audioContext, baseFreq, n, l, m, Z) => {
    const resonators = [];
    
    for (let mode = 0; mode < n + l; mode++) {
      ...
      const scaleStep = quantumToScaleStep(n, l, m, Z) + mode;
      const freq = getScaleFrequency(baseFreq, scaleStep) * (1 + Math.random() * 0.1);
      
      filter.type = 'bandpass';
      filter.frequency.setValueAtTime(freq, audioContext.currentTime);
      filter.Q.setValueAtTime(20 + l * 5, audioContext.currentTime);
      
      delay.delayTime.setValueAtTime(Math.max(0.001, 1 / freq), audioContext.currentTime);
  ...
  const createCinematicSynth = (audioContext, baseFreq, n, l, m, Z) => {
    const layers = [];
    ...
    const subFreq = getScaleFrequency(baseFreq, quantumToScaleStep(n, l, m, Z) - 12); // One octave down
    
    subOsc.type = 'sine';
    subOsc.frequency.setValueAtTime(subFreq, audioContext.currentTime);
    subFilter.type = 'lowpass';
    subFilter.frequency.setValueAtTime(subFreq * 3, audioContext.currentTime);
    ...
    // Harmonic layers
    const padOscs = [];
    for (let harmonic = 0; harmonic < 6; harmonic++) {
      const osc = audioContext.createOscillator();
      const gain = audioContext.createGain();
      const filter = audioContext.createBiquadFilter();
      const panner = audioContext.createStereoPanner();
      ...
      panner.pan.setValueAtTime((m / (2 * l + 1)) * spatialWidth, audioContext.currentTime);
      gain.gain.setValueAtTime(volume * 0.15 / (harmonic + 1), audioContext.currentTime);
    ...
    const grainScheduler = () => {
      if (Math.random() < 0.3 + harmonicContent * 0.4) {
        ...
        const randomScaleStep = quantumToScaleStep(n, l, m, Z) + Math.floor(Math.random() * 12);
        const freq = getScaleFrequency(baseFreq, randomScaleStep);
  ...
  const generatePointCloud = useCallback(() => {
    ...
    if (coordinateSystem === 'spherical') {
      ...
      let attempts = 0;
      const maxAttempts = targetPoints * 40;
      
      while (pointsGenerated < targetPoints && attempts < maxAttempts) {
        attempts++;
        
        // Enhanced spherical sampling
        const u = Math.random();
        const v = Math.random();
        const w = Math.random();
        ...
        let threshold = Math.pow(density, 0.2) * 200;
        if (n > 4) threshold *= (n / 4);
  ...
  const updateVisualization = useCallback(() => {
    ...
    switch (visualizationMode) {
      ...
      case 'spheres':
        ...
        const instanceCount = Math.min(points.length / 3, 50000);
      ...
      case 'wireframe':
        ...
        // Fixed wireframe connections
        const lineGeometry = new THREE.BufferGeometry();
        const linePositions = [];
        const lineColors = [];
        
        const maxConnections = Math.min(wireframeConnections, 8);
        const connectionDistance = 1.2 + sphereSize * 10;
        ...
        for (let i = 0; i < points.length; i += 3) {
          ...
          for (let j = i + 3; j < points.length && connections < maxConnections; j += 3) {
            ...
            if (dist < connectionDistance) {
              ...
              const alpha = Math.exp(-dist * 1.5) * (glowIntensity + 0.3);
              lineColors.push(
          ...
          layers.forEach(layer => {
            if (layer.padOscs) {
              layer.padOscs.forEach(({ filter, gain }) => {
                if (filter && filter.frequency && gain && gain.gain) {
                  try {
                    filter.frequency.setValueAtTime(filter.frequency.value * modulation, audioContextRef.current.currentTime);
                    gain.gain.setValueAtTime(Math.max(0, gain.gain.value * modulation), audioContextRef.current.currentTime);
                  } catch (e) {
  ...
  }, [quantumNumbers, element, synthesisMode, musicalScale, rootNote]);
    ...
    <div className="w-full h-screen bg-black text-white flex flex-col">
      ...
      <div className="p-2 bg-gray-900 border-b border-gray-700 flex justify-between items-center">
        <div className="flex-1">
          <h1 className="text-lg font-bold text-center">Quantum Orbital Sonification Studio</h1>
          <p className="text-center text-gray-300 text-xs">Musical scales • Frequency ranges • Complete synthesis modes • Fixed wireframe</p>
        </div>
        <button
      ...
      <div className="flex-1 flex flex-col lg:flex-row">
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute top-3 right-3 bg-black bg-opacity-90 p-2 rounded-lg backdrop-blur-sm">
            <div className="text-xs space-y-1">
              ...
              <div>Scale: {musicalScales[musicalScale].name}</div>
              <div>Root: {rootNote}</div>
              <div>Range: {frequencyRange.min}-{frequencyRange.max}Hz</div>
        ...
        <div className="w-full lg:w-80 bg-gray-800 p-3 space-y-3 overflow-y-auto text-xs">
          ...
          {/* Musical Controls */}
          <div className="space-y-2">
            <h3 className="text-sm font-semibold">Musical Settings</h3>
            
            <div>
              <label className="block text-xs mb-1">Musical Scale</label>
              <select
                value={musicalScale}
                onChange={(e) => setMusicalScale(e.target.value)}
                className="w-full p-1 bg-gray-700 rounded text-xs"
              >
                {Object.entries(musicalScales).map(([key, scale]) => (
            ...
            <div className="grid grid-cols-2 gap-1">
              <div>
                <label className="block text-xs mb-1">Root Note</label>
                <select
                  value={rootNote}
            ...
            <div className="grid grid-cols-2 gap-1">
              <div>
                <label className="block text-xs mb-1">Min Freq: {frequencyRange.min}Hz</label>
                <input
                  type="range"
                  min="20"
                  max="1000"
                  ...
                  onChange={(e) => setFrequencyRange(prev => ({ ...prev, min: parseInt(e.target.value) }))}
                  className="w-full h-1"
                />
              </div>
              
              <div>
                <label className="block text-xs mb-1">Max Freq: {frequencyRange.max}Hz</label>
                <input
                  type="range"
                  min="100"
                  max="4000"
                  ...
                  onChange={(e) => setFrequencyRange(prev => ({ ...prev, max: parseInt(e.target.value) }))}
                  className="w-full h-1"
                />
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-1">
              <div>
                <label className="block text-xs mb-1">Size: {sphereSize.toFixed(3)}</label>
                <input
                  type="range"
                  min="0.01"
                  max="0.2"
              ...
              <div>
                <label className="block text-xs mb-1">Vol Density: {volumeDensity.toFixed(1)}</label>
                <input
                  type="range"
                  min="0.1"
                  max="2"
              ...
              <div>
                <label className="block text-xs mb-1">Wire Links: {wireframeConnections}</label>
                <input
                  type="range"
                  min="1"
                  max="8"
            ...
            <div className="grid grid-cols-2 gap-1">
              <div>
                <label className="block text-xs mb-1">Light: {lightIntensity.toFixed(1)}</label>
                <input
                  type="range"
                  min="0.1"
                  max="2"
      ...
      <div className="p-1 bg-gray-900 text-center text-xs text-gray-400">
        ...
        {elements[element].name} • {musicalScales[musicalScale].name} in {rootNote} • 
        {synthesisMode} synthesis • {evolutionType} evolution • 
